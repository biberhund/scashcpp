...
Value getstakinginfo(const Array& params, bool fHelp)
{
    ...
    int nExpectedTime = (staking && (nTargetSpacing > 0)) ? (nTargetSpacing * nNetworkWeight / nWeight) : -1;
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Scash is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Scash is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Scash is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Scash is downloading blocks...");
