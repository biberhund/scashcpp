...
const string strMessageMagic = "Scash Signed Message:\n";
...
static const int cutoffSeriesLength = 5;
static std::vector<int> x13CutoffHeight(cutoffSeriesLength);
static std::vector<int> x13CutoffReward(cutoffSeriesLength);
static std::vector<int> mSHACutoffHeight(cutoffSeriesLength);
static std::vector<int> mSHACutoffReward(cutoffSeriesLength);
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // Scash: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (unsigned int i = 0; i < vout.size(); i++)
    {
        ...
        // Scash: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Scash: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!(IsCoinBase() || IsCoinStake()))
        return 0;
    return max(0, (nCoinbaseMaturity+COINBASE_MATURITY_SAFE_GAP) - GetDepthInMainChain());
}
...
// Scash: find block wanted by given orphan block
...
int64 GetProofOfWorkReward(int height, int64 nFees, uint256 prevHash, bool mSHAGenerated)
{
    int result = 0;
    for (size_t u = 0; u < x13CutoffHeight.size(); u++)
    {
        if (height < x13CutoffHeight[u])
        {
            result += x13CutoffReward[u];
            break;
        }
    }
    ...
    if (mSHAGenerated)
    {
        for (size_t u = 0; u < mSHACutoffHeight.size(); u++)
        {
            if (height < mSHACutoffHeight[u])
            {
                result += mSHACutoffReward[u];
                break;
            }
        }
    }
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight, bool mSHAGenerated)
{
    int64 nRewardCoinYear;
    nRewardCoinYear = MAX_CLOAK_PROOF_OF_STAKE;
    int64 nSubsidy = nCoinAge * nRewardCoinYear / 365;

    if (fDebug && (GetBoolArg("-printcreation") || fDumpAll))
    {
...
// Scash: find last block index up to pindex
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Scash: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // Scash: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // Scash: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Scash: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Scash
    bool fStrictPayToScriptHash = true; // Always active in Scash
    ...
    // Scash: track money supply and mint amount info
    pindex->nMint = nValueOut - nValueIn + nFees;
    pindex->nMoneySupply = (pindex->pprev? pindex->pprev->nMoneySupply : 0) + nValueOut - nValueIn;
    if (!txdb.WriteBlockIndex(CDiskBlockIndex(pindex)))
        return error("Connect() : WriteBlockIndex for pindex failed");

    // Scash: fees are not collected by miners as in bitcoin
    // Scash: fees are destroyed to compensate the entire network
    if (fDebug && (GetBoolArg("-printcreation") || fDumpAll))
...
bool CBlock::SetBestChain(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    if (!fIsInitialDownload)
    {
        ...
        {
            if (pindex->nVersion > CBlock::SIMPLE_VERSION)
                ++nUpgraded;
            pindex = pindex->pprev;
        }
        if (nUpgraded > 0)
            printf("SetBestChain: %d of last 100 blocks above version %d\n", nUpgraded, CBlock::SIMPLE_VERSION);
        if (nUpgraded > 100/2)
...
// Scash: total coin age spent in transaction, in the unit of coin-days.
...
bool CTransaction::GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        if (fDebug && (GetBoolArg("-printcoinage") || fDumpAll))
...
// Scash: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // Scash: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // Scash: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit(pindexNew->nHeight)))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // Scash: record proof-of-stake hash value
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Scash: only the second transaction can be the optional coinstake
    for (unsigned int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // SCash: coinbase output should be empty if proof-of-stake block
...
bool CBlock::AcceptBlock()
{
    ...
    // Scash: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Scash: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // Scash: getblocks may not obtain the ancestor block rejected
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Scash", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
LoadBlockIndexResult LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        block.nVersion = 1;
        block.nTime    = 1491832568;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1317033;
...
string GetWarnings(string strFor)
{
    ...
    // Scash: should not enter safe mode for longer invalid chain
    ...
    // Scash: if detected invalid checkpoint enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // Scash: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // Scash: record my external IP reported by peer
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // Scash: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // Scash: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    // Scash: if coinstake available add coinstake tx
...
//                        if (fDebug) assert("mempool transaction missing input" == 0);
        ...
        while (!vecPriority.empty())
        {
            ...
            // Scash: simplify transaction fee - allow free = false
            ...
            if (fDebug && (GetBoolArg("-printpriority") || fDumpAll))
            {
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // Scash: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
