...
// We now get our external IP from some servers first and only use this as a backup
// TODO: improve
...
bool CNode::Misbehaving(int howmuch)
{
    ...
    {
        int64 banTime = GetTime()+GetArg("-bantime", 60*60*1);  // Default 1-hour ban
...
void ThreadMapPort2(void* parg)
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Scash " + FormatFullVersion();
#ifndef UPNPDISCOVER_SUCCESS
        /* miniupnpc 1.5 */
...
void ThreadOpenConnections2(void* parg)
{
    ...
    LOOP
    {
        ...
        static unsigned int maxConnectionsBurst = GetArg("-burstconnect", 3);
...
void ThreadOpenAddedConnections2(void* parg)
{
    ...
    {
        LOCK(_cs_customNodes);
        _addedNodes = mapMultiArgs["-addnode"];
    }
    ...
    if (HaveNameProxy()) {
        while(!fShutdown) {
            ...
            {
                LOCK(_cs_customNodes);
                BOOST_FOREACH(string& strAddNode, _addedNodes)
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Scash is probably already running."), addrBind.ToString().c_str());
        else
