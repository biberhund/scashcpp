...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "Scash:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("Scash"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("Scash"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        ...
        "  -dumpall               " + _("Dumps all possible debug information (Warning: debug file grows way too fast)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  Scash is probably already running."), strDataDir.c_str()));
    ...
    if (mapArgs.count("-reservebalance")) // Scash: reserve balance amount
    {
    ...
    if (mapArgs.count("-checkpointkey")) // Scash: checkpoint master priv key
    {
