...
// Scash: optional setting to unlock wallet for block minting only;
...
// Scash: total coins staked (non-spendable until maturity)
...
bool CWallet::SelectCoinsMinConf(int64 nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
    ...
    BOOST_FOREACH(COutput output, vCoins)
    {
        ...
        if (pcoin->nTime > nSpendTime)
            continue;  // Scash: timestamp must not exceed spend time
...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64> >& vecSend, CWalletTx& wtxNew,
                                ...
                                const CCoinControl* coinControl, std::string payName,
                                int messageBytes)
{
    int64 nValue = 0;
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            LOOP
            {
                ...
                // Scash: sub-cent change is moved to fee
                                ...
                                int messageBytes)
{
    ...
    return CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, coinControl, payName, messageBytes);
}
...
bool CWallet::GetStakeWeight(const CKeyStore& keystore, uint64& nMinWeight, uint64& nMaxWeight, uint64& nWeight)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        CBigNum bnCoinDayWeight = CBigNum(pcoin.first->vout[pcoin.second].nValue) * nTimeWeight / COIN / (4 * 60 * 60);
...
// Scash: create coin stake transaction
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64 nSearchInterval, CTransaction& txNew)
{
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        for (unsigned int n=0; n<min(nSearchInterval,(int64)nMaxStakeSearchInterval) && !fKernelFound && !fShutdown; n++)
        {
            ...
            {
               // Found a kernel
                if (fDebug && (GetBoolArg("-printcoinstake") || fDumpAll))
    ...
    LOOP
    {
        ...
        {
            if (fDebug && (GetBoolArg("-printfee") || fDumpAll))
...
string CWallet::SendMoney(CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew, bool fAskFee, int messageBytes)
{
    ...
    if (!CreateTransaction(scriptPubKey, nValue, wtxNew, reservekey, nFeeRequired, NULL, "", messageBytes))
    {
        string strError;
...
string CWallet::SendMoneyToDestination(const CTxDestination& address, int64 nValue, CWalletTx& wtxNew, bool fAskFee, int messageBytes)
{
    // Check amount
    if (nValue <= 0)
    ...
    return SendMoney(scriptPubKey, nValue, wtxNew, fAskFee, messageBytes);
}
...
void CWallet::ReserveKeyFromKeyPool(int64& nIndex, CKeyPool& keypool)
{
    ...
    keypool.vchPubKey = CPubKey();
    {
        ...
        if (fDebug && (GetBoolArg("-printkeypool") || fDumpAll))
...
// Scash: check 'spent' consistency between wallet and txindex
// Scash: fix wallet spent state according to txindex
...
// Scash: disable transaction (only for coinstake)
