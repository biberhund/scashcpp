...
static const uint64_t PAGE_GRANULARITY = 1048576UL; // each page is 8Mb
static const uint64_t PAGES_COUNT = ((32UL * 1UL) / 8UL);
...
static void reductionF_verboseTest() {
	uint64_t limit = RED_LIMIT_START;
	uint64_t part = RED_PART_START;
	uint64_t space = RED_SPACE_START;
...
static uint64_t reductionF(uint64_t s0, uint64_t s) {
...
static uint64_t sha3Mem64(uint64_t s);
 ...
 */
static void keccakfExt(uint64_t s[25], bool extendedVersion)
{
    int i, j, round;
    uint64_t t, bc[5];
...
static void
sha3_Update(void *priv, void const *bufIn, size_t len, bool extendedVersion = false)
{
    sha3_context *ctx = (sha3_context *) priv;
 ...
 */
static void const *
sha3_Finalize(void *priv, bool extendedVersion = false)
{
    sha3_context *ctx = (sha3_context *) priv;
...
static const int UnitSize = 8; // assert == sizeof(uint64_t)
static const int HashSizeInUints = 8; // 512bits / 8 / sizeof(uint64_t)

static uint64_t sha3UnMem64(uint64_t s) {	
	sha3_context c;
	const uint64_t *hash;

	sha3_Init512(&c);
	for (int i = 0; i < sizeof(sha3m_buf); i += UnitSize) {
		memcpy_s(&sha3m_buf[i], UnitSize, &s, sizeof(s));
	}
	sha3_Update(&c, &sha3m_buf, sizeof(sha3m_buf), false);
	hash = (const uint64_t*)sha3_Finalize(&c, false);
...
static uint64_t sha3Mem64(uint64_t s) {
	...
	uint64_t result = sha3UnMem64(s);
	//printf("sha3Mem64 %llu: %llu\n", s, result);
...
static void precompute()
{
	for (int p = 0; p < PAGES_COUNT; p++) {
		...
		{
			printf("Loading hash table page %i of %llu...\n", p, PAGES_COUNT);
			std::fstream myFile(fName, std::ios::in | std::ios::binary);
			myFile.read((char*)PrecomputedTable[p], PAGE_GRANULARITY * sizeof(uint64_t));
			myFile.close();
		}
		else
		{
			printf("Filling hash table page %i of %llu...\n", p, PAGES_COUNT);
			for (int j = 0; j < PAGE_GRANULARITY; j++) {
				PrecomputedTable[p][j] = sha3UnMem64(p * PAGE_GRANULARITY + j);
			}
		}

		std::fstream myFile(fName, std::ios::out | std::ios::binary);
		myFile.write((const char*)PrecomputedTable[p], PAGE_GRANULARITY * sizeof(uint64_t));
		myFile.close();
	}	
}
...
int main()
{
	...
	MeasureStart();
	PrecomputedTable = new uint64_t*[PAGES_COUNT];
	for (int i = 0; i < PAGES_COUNT; i++)
		PrecomputedTable[i] = new uint64_t[PAGE_GRANULARITY];
	precompute();
	...
	MeasureStart();
	sha3_Init512(&c);
	hash = (const uint8_t*)sha3_Finalize(&c, true);
    ...
    sha3_Init512(&c);
    sha3_Update(&c, buf, sizeof(buf), true);
    hash = (const uint8_t*)sha3_Finalize(&c, true);
    ...
    sha3_Init512(&c);
    sha3_Update(&c, buf, sizeof(buf) / 2, true);
    sha3_Update(&c, buf + sizeof(buf) / 2, sizeof(buf) / 2, true);
    hash = (const uint8_t*)sha3_Finalize(&c, 1);
	if (toHex(hash, 64) != "8FF228D737DB31D472A94F6AD85A508402DC21E05113274FCCC1E2EC9727B3BBE478D1032A9F2D4A807C24C698FBB401B47254E8AEDD22D8D2EE1C2EB9950E56") {
	...
	for (unsigned int u = 0; u < 1024; u++) {
		sha3_Init512(&c);
		sprintf_s((char*)buf, sizeof(buf), "%u", u);
		sha3_Update(&c, buf, sizeof(buf), true);
		hash = (const uint8_t*)sha3_Finalize(&c, true);
	}
	MeasureEnd(1024);
	...
	for (unsigned int u = 1024; u < 16 * 1024; u++) {
		sha3_Init512(&c);
		sprintf_s((char*)buf, sizeof(buf), "%u%u%u%u%u%u%u%u", u, u, u, u, u, u, u, u);
		sha3_Update(&c, buf, sizeof(buf), true);
		hash = (const uint8_t*)sha3_Finalize(&c, true);
	}
	MeasureEnd(15 * 1024);
